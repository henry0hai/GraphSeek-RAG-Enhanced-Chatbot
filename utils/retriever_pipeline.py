...
from utils.enhanced_processing import QueryProcessor, AdvancedRetriever, ResultPostProcessor
...
def retrieve_documents(query, uri, model, chat_history=""):
    ...
    if intent in ['list', 'count']:
        ...
        dynamic_max_contexts = max(base_max_contexts * 2, 15)  # At least 15 for count/list
        st.write(f"� **Smart Context Boost**: Increased from {base_max_contexts} to {dynamic_max_contexts} for {intent.upper()} query")
    elif intent == 'summary':
    ...
    if st.session_state.enable_reranking and st.session_state.retrieval_pipeline.get("reranker"):
        st.write("\n🔍 **Step 6: Enhanced Re-ranking**")
